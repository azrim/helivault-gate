...
const Mint = () => {
  ...
  const maxSupplyNum =
    typeof maxSupply === "bigint" ? Number(maxSupply) : 10000;
  const mintPriceHVT =
    typeof mintPriceResult === "bigint" ? formatEther(mintPriceResult) : "0";
  ...
  return (
    <Page
      title="Mint NFT"
      description="Become a part of the Helivault ecosystem by minting a unique NFT on the Helios blockchain."
    >
      <div className="space-y-16 pb-24">
        <section className="text-center pt-24 pb-12">
          ...
          >
            Become a part of the Helivault ecosystem by minting a unique NFT on
            the Helios blockchain.
          </motion.p>
        </section>

        <section className="grid lg:grid-cols-5 gap-12">
          <motion.div
            className="lg:col-span-3 aspect-square rounded-2xl bg-card p-6"
          ...
          >
            <Card>
              <CardHeader>
                <CardTitle>
                  {(nftName as string) || <Skeleton className="h-6 w-3/4" />}
                </CardTitle>
              </CardHeader>
              <CardContent>
                <p className="text-muted-foreground">
                  A relic from a bygone era, pulsating with the energy of the
                  cosmos. It is said to hold the key to unlocking the secrets of
                  the Helivault.
                </p>
              </CardContent>
            </Card>
              ...
              <CardContent className="space-y-6">
                ...
                <div className="flex gap-4">
                  <Input
                    ...
                    disabled={isLoading}
                  />
                  <div className="flex-1">{renderMintButton()}</div>
                </div>
              </CardContent>
            </Card>
            <Card>
              ...
              <CardContent className="space-y-3 text-sm">
                <div className="flex justify-between">
                  ...
                  <div className="flex items-center gap-2 font-mono">
                    <a
                      href={`${heliosTestnet.blockExplorers.default.url}/address/${contractConfig.address}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="hover:underline"
                    >{`${contractConfig.address.slice(0, 6)}...${contractConfig.address.slice(-4)}`}</a>
                    <Button
                      variant="ghost"
                      size="icon"
                      className="h-6 w-6"
