...
export const DeploymentProvider = ({ children }: { children: ReactNode }) => {
  ...
  const setIsDeploying = (isDeploying: boolean) =>
    setState((prev) => ({ ...prev, isDeploying }));
  const setDeploymentCompleted = (isCompleted: boolean) =>
    setState((prev) => ({ ...prev, deploymentCompleted: isCompleted }));
  const setDeployedContracts = (contracts: Record<string, string>) =>
